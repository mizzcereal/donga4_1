...
min_population = districts[3]

for district in districts:       
    
    for min_pop in districts:
        if(min_pop['population'] < min_population['population']):
            min_population = min_pop 
        

    folium.CircleMarker(
        location=[district['lat'],district['lon']],
        radius= district['population']/10000,color='blue', fill=True,fill_color='blues',
        fill_opacity=0.6,
    ...
    folium.Marker(
        location=[district['lat'], district['lon']],
        icon=folium.DivIcon(icon_anchor=(0, 0),
                            html=html_start + district['name'] + html_end)).add_to(map_pusan)
    
    folium.Marker(
        location=[min_population['lat'], min_population['lon']], 
        icon=folium.Icon(icon='flag',color='red')).add_to(map_pusan)
